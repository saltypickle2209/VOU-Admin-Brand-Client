...
import { CollectableItem, ItemSet } from '@/app/lib/definition';
import { createItemCollecting, ItemCollectingFormState } from '@/app/lib/action';

// remember to add voucher list props
export default function ItemCollectingForm({ voucherData }: { voucherData: any }) {
    const initialState: ItemCollectingFormState = { message: null, errors: {} }
    const [state, formAction] = useFormState(createItemCollecting, initialState)
    ...
    const [collectableItems, setCollectableItems] = useState<CollectableItem[]>(Array(6).fill({
        image: null,
        name: '',
        description: '',
        ratio: ''
